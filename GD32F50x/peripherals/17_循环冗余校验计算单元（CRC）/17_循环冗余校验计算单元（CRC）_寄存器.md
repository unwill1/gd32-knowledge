...
## 17.4.1. 数据寄存器（CRC_DATA）

地址偏移：0x00

复位值：0xFFFF FFFF

该寄存器按字（32 位）读访问，按字节（8 位）、半字（16 位）或字（32 位）写访问。
