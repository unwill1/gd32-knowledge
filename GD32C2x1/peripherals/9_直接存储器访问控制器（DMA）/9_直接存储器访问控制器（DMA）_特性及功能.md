...
DMA 控制器提供了一种硬件传输方式，在外设和存储器之间或者存储器和存储器之间传输数据，而无需 CPU 的介入，从而使 CPU 可以专注在处理其他系统功能上。DMA 控制器有 3 个通道。每个通道都是专门用来处理一个或多个外设的存储器访问请求的。DMA 控制器内部实现了一个仲裁器，用来仲裁多个 DMA请求的优先级。
...
![image](images/53a69aadadbf.jpg)


由 9-1. DMA 所示，DMA控制器由 4 部分组成：
...
假设 DMA_CHxCNT 寄存器的值为 4，并且 PNAGA 和 MNAGA 位均置位。结合 PWIDTH 和MWIDTH 的各种配置，DMA 传输的操作详见 9-1. DMA 。
...
9-2. 中详细描述了DMA控制器与外设之间的握手机制。


图 9-2. 握手机制


![image](images/180d7dd923d2.jpg)
...
每一个中断事件在DMA_INTF寄存器中有专用的标志位，在DMA_INTC寄存器中有专用的清除位，在DMA_CHxCTL寄存器中有专用的使能位。 9-2. 描述了其对应关系。
...
DMA中断逻辑如 9-3. DMA 所示，任何类型中断使能时，产生了相应中断事件均会产生中断。


图 9-3. DMA 中断逻辑图


![image](images/5619e4a6bad0.jpg)
...
每个 DMA通道的请求都连接至由 DMAMUX 请求复用器的对应通道输出来转发的 AHB / APB外设请求，参考 10-2.GD32C2x1 DMAMUX 。
