...
数据输入寄存器用于传输明文或密文数据块到输入 中进行处理。首先写入 的是数据块的 ，最后才是 。当 位为 ，并且输入 非空时，读取该寄存器时返回中的首个字。当CAUEN位为1时，读取该寄存器返回一个不确定的值。一旦执行了读操作，则必须要刷新FIFO以处理新数据块。
...
<table><tr><td>位/位域</td><td>名称</td><td>描述</td></tr><tr><td>31:0</td><td>CTXx[31:0]</td><td>CAU处理器的内部状态信息。当有一个更高优先级的任务需要处理时,读取并保存这些寄存器的数据,恢复的时候将保存的数据写回到这些寄存器从而恢复前面被挂起的任务。注意:这些寄存器只能在CCM、CMAS、或CCM模式下使用。</td></tr></table>
