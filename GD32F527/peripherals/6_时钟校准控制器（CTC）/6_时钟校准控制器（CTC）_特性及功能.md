...
CTC 模块的内部结构图如图5-1. CTC简介。


图6-1. CTC简介


![image](images/1871f5712ef3.jpg)
...
■3 $\mathsf { 3 } \times \mathsf { C K L I M } \leq \mathsf { C o u n t e r } < 1 2 8 \times \mathsf { C K L I M H } \bar { 3 }$ ，检测到REF同步脉冲信号；
...
$$
\text { RLVALUE } = (F \_ c l o c k \div F \_ R E F) - 1\tag{式6-1}
$$

CKLIM的值由用户根据时钟的精度来设置，一般建议设为步长的一半，所以CKLIM的值为：

$$
\mathrm{CKLIM} = \left(\mathrm{F} _ {\text { clock }} \div \mathrm{F} _ {\text { REF }}\right) \times 0.12 \% \div 2\tag{式6-2}
$$
