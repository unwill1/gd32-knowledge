...
![image](images/50adbc2ee2b1.jpg)


## 21.4. SPI 信号线描述

## 21.4.1. 常规配置（非 SPI四线模式）
...
在主机模式（MSTMOD=1）下，如果应用程序使用多主机连接方式，NSS可以配置为硬件输入模式（SWNSSEN=0, NSSDRV=0）或者软件模式（SWNSSEN=1）。一旦 NSS 引脚（在硬件 NSS 模式下）或 SWNSS 位（在软件 NSS模式下）被拉低，SPI 将自动进入从机模式，并且产生主机配置错误，CONFERR 位置 1。
...
5. 按照上文NSS 的描述，根据应用程序的需求，配置NSS模式（SPI_CTL0中的SWNSSEN位和SPI_CTL1中的NSSDRV位）。

6. 如果工作在TI模式，需要将SPI_CTL1中的TMOD位置1，否则，忽略此步骤。

7. 根据 21-5. SPI ，配置MSTMOD位、RO位、BDEN位和BDOEN位。
...
在最后一个采样时钟边沿之后，接收到的数据将从移位寄存器存入到接收缓冲区，且 RBNE（接收缓冲区非空）位置 1。软件通过读 SPI_DATA 寄存器获得接收的数据，此操作会自动清除RBNE 标志位。在 MRU 和 MRB 模式中，为了接收下一个数据帧，硬件需要连续发送时钟信号，而在全双工主机模式（MFD）中，当发送缓冲区非空时，硬件才接收下一个数据帧。
...
相比于发送模式的情况，主机接收模式（MRU 或 MRB）与全双工的接收流程大不相同。在MRU 模式或 MRB模式下，在 SPI 使能后，SPI 产生连续的 SCK信号，直到 SPI 停止。所以，软件应该忽略 TBE 标志位，并且在 RBNE 位置 1 后，读出接收缓冲区内的数据，否则，将会产生接收过载错误。
...
$$
T _ {d} = \frac {T _ {\text { bit }}}{2} + 5 ^ {*} T _ {\text { pclk }} \tag {21-1}
$$

例如，如果 PSC[2:0] = 010，那么 $T _ { d }$ 数值为 9*Tpclk。
...
图 21-11. SPI 四线模式写操作时序图


![image](images/1218d51a1f26.jpg)
...
图 21-12. SPI 四线模式读操作时序图


![image](images/c2e7ff9d5b30.jpg)
...
SPI 使能后，如果 DMATEN 位置 1，每当 TBE=1 时，SPI 将会发出一个 DMA 请求，然后 DMA应答该请求，并自动写数据到 SPI_DATA 寄存器。如果 DMAREN 位置 1，每当 RBNE=1 时，SPI 将会发出一个 DMA请求，然后 DMA 应答该请求，并自动从 SPI_DATA 寄存器读取数据。
...
当配置 SPI 为从模式并且使用 CRC 功能时，即使 NSS 引脚为高时仍然会执行 CRC 的计算(当 NSS 信号为高时，只要 SCK 引脚上有时钟脉冲，则 CRC 计算会继续执行)。当主设备交替地与多个从设备进行通信时，将会出现这种情况,此时建议在 NSS 信号为低时重启 CRC 功能)。当从设备未选中(NSS 信号为高)转换到被选中为一个新的从设备(NSS 信号为低)的时候，为了保持主从设备端下次 CRC 计算结果的同步，应该清除主从两端的 CRC 数值。建议按照下述步骤清除 CRC 数值：
...
I2S 音频标准是通过设置 SPI_I2SCTL 寄存器中的 I2SSTD[1:0]位域来选择的，可以选择四种音频标准：I2S 飞利浦标准，MSB对齐标准，LSB对齐标准和 PCM 标准。除 PCM 之外的所有标准都是两个通道(左通道和右通道)的音频数据分时复用 I2S 接口的，并通过 I2S_WS 信号来区分当前数据属于哪个通道。对于 PCM 标准，I2S_WS 信号表示帧同步信息。
...
图 21-14. I2S 飞利浦标准时序图（DTLEN=00, CHLEN=0, CKPL=0）


![image](images/863a37b459bf.jpg)



图 21-15. I2S 飞利浦标准时序图（DTLEN=00, CHLEN=0, CKPL=1）


![image](images/34c1ccac932d.jpg)


当 16 位数据打包成 16 位数据帧时，每完成一帧数据的传输只需要访问 SPI_DATA 寄存器一次。


图 21-16. I2S 飞利浦标准时序图（DTLEN=10, CHLEN=1, CKPL=0）


![image](images/c59d5782600f.jpg)



图 21-17. I2S 飞利浦标准时序图 $( { \bf D } { \sf T } { \sf L } { \sf E N } { = } 1 0 , { \sf C } { \sf H } { \sf L } { \sf E N } { = } 1 , { \sf C } { \sf K } { \sf P } { \sf L } { = } 1 )$


![image](images/62bd27a16a6c.jpg)
...
图 21-18. I2S 飞利浦标准时序图（DTLEN=01, CHLEN=1, CKPL=0）


![image](images/4557fd47854a.jpg)



图 21-19. I2S 飞利浦标准时序图（DTLEN=01, CHLEN=1, CKPL=1）


![image](images/572eb5c72b4c.jpg)
...
图 21-20. I2S 飞利浦标准时序图（DTLEN=00, CHLEN=1, CKPL=0）


![image](images/769ac7a55918.jpg)



图 21-21. I2S 飞利浦标准时序图（DTLEN=00, CHLEN=1, CKPL=1）


![image](images/94edc5a91e06.jpg)
...
图 21-22. MSB 对齐标准时序图（DTLEN=00, CHLEN=0, CKPL=0）


![image](images/9d01be645c8a.jpg)



图 21-23. MSB 对齐标准时序图 $( { \tt D T L E N } { = } { 0 } 0 , { \tt C H L E N } { = } 0 , { \tt C K P L } { = } 1 )$


![image](images/30fd97d84214.jpg)



图 21-24. MSB 对齐标准时序图 $( { \bf D } { \sf T } { \sf L } { \sf E N } { = } 1 0 , { \sf C } { \sf H } { \sf L } { \sf E N } { = } 1 , { \sf C } { \sf K } { \sf P } { \sf L } { = } 0 )$


![image](images/fe256c99d538.jpg)



图 21-25. MSB 对齐标准时序图 $( { \tt D T L E N } { = } 1 0 , { \tt C H L E N } { = } 1 , { \tt C K P L } { = } 1 )$


![image](images/63348b23ebc9.jpg)



图 21-26. MSB 对齐标准时序图 $( { \bf D } { \sf T } { \sf L } { \sf E N } { = } { \bf 0 } 1 , { \sf C } { \sf H } { \sf L } { \sf E N } { = } 1 , { \sf C } { \sf K } { \sf P } { \sf L } { = } { \bf 0 } )$


![image](images/eca7f1259620.jpg)



图 21-27. MSB 对齐标准时序图 $( { \tt D T L E N } { = } { 0 1 } , { \tt C H L E N } { = } 1 , { \tt C K P L } { = } 1 )$


![image](images/25f31021b1eb.jpg)



图 21-28. MSB 对齐标准时序图 $( { \bf D } { \sf T } { \sf L } { \sf E N } { = } { \bf 0 } 0 , { \sf C } { \sf H } { \sf L } { \sf E N } { = } 1 , { \sf C } { \sf K } { \sf P } { \sf L } { = } 0 )$


![image](images/b24bc5945562.jpg)



图 21-29. MSB 对齐标准时序图（DTLEN=00, CHLEN=1, CKPL=1）


![image](images/392e12d04699.jpg)
...
图 21-30. LSB 对齐标准时序图（DTLEN=01, CHLEN=1, CKPL=0）


![image](images/3f6b145247ee.jpg)



图 21-31. LSB 对齐标准时序图（DTLEN=01, CHLEN=1, CKPL=1）


![image](images/c0f5ad91754a.jpg)
...
图 21-32. LSB 对齐标准时序图（DTLEN=00, CHLEN=1, CKPL=0）


![image](images/93503ddb4d7b.jpg)



图 21-33. LSB 对齐标准时序图（DTLEN=00, CHLEN=1, CKPL=1）


![image](images/0cb6d5401dd9.jpg)
...
图 21-34. PCM 标准短帧同步模式时序图 $( { \tt D T L E N } { = } { 0 } 0 , { \tt C H L E N } { = } 0 , { \tt C K P L } { = } 0 )$


![image](images/c7064bbfa56f.jpg)



图 21-35. PCM 标准短帧同步模式时序图 $( \mathsf { D } \mathsf { T } \mathsf { L } \mathsf { E } \mathsf { N } { = } 0 0 , \mathsf { C } \mathsf { H } \mathsf { L } \mathsf { E } \mathsf { N } { = } 0 , \mathsf { C } \mathsf { K } \mathsf { P } \mathsf { L } { = } 1 )$ ）


![image](images/8ddd975d9ed4.jpg)



图 21-36. PCM 标准短帧同步模式时序图 $( { \bf D } { \bf T } { \bf L } { \sf E N } { = } 1 0 , { \sf C } { \sf H } { \bf L } { \sf E N } { = } 1 , { \sf C } { \sf K } { \sf P } { \bf L } { = } 0 )$


![image](images/93880557b8be.jpg)



图 21-37. PCM 标准短帧同步模式时序图 $( { \tt D T L E N } { = } 1 0 , { \tt C H L E N } { = } 1 , { \tt C K P L } { = } 1 )$


![image](images/7548501b4422.jpg)



图 21-38. PCM 标准短帧同步模式时序图（DTLEN=01, CHLEN=1, CKPL=0）


![image](images/6a7c81122f52.jpg)



图 21-39. PCM 标准短帧同步模式时序图（DTLEN=01, CHLEN=1, CKPL=1）


![image](images/3e806812e5d9.jpg)



图 21-40. PCM 标准短帧同步模式时序图（DTLEN=00, $C H L E N = 1$ , $\mathsf { C K P L } \mathbf { = } \mathbf { 0 } )$ ）


![image](images/3311e6ded59e.jpg)



图 21-41. PCM 标准短帧同步模式时序图（DTLEN=00, CHLEN=1, ${ \mathsf { C K P L } } { \mathsf { = } } 1 )$ ）


![image](images/319d17dea757.jpg)


长帧同步模式的各种配置情况时序图如下所示。


图 21-42. PCM 标准长帧同步模式时序图 $( { \tt D T L E N } { = } { 0 } 0 , { \tt C H L E N } { = } 0 , { \tt C K P L } { = } 0 )$ ）


![image](images/85d00711a7f2.jpg)



图 21-43. PCM 标准长帧同步模式时序图 $( \mathsf { D } \mathsf { T } \mathsf { L } \mathsf { E } \mathsf { N } { = } 0 0 , \mathsf { C } \mathsf { H } \mathsf { L } \mathsf { E } \mathsf { N } { = } 0 , \mathsf { C } \mathsf { K } \mathsf { P } \mathsf { L } { = } 1 )$ ）


![image](images/c99e3a078991.jpg)



图 21-44. PCM 标准长帧同步模式时序图 $( { \tt D T L E N } { = } 1 0 , { \tt C H L E N } { = } 1 , { \tt C K P L } { = } 0 )$


![image](images/bc0313de2099.jpg)



图 21-45. PCM 标准长帧同步模式时序图 $( { \mathsf { D } } { \mathsf { T } } { \mathsf { L } } { \mathsf { E N } } { = } 1 0 , { \mathsf { C } } { \mathsf { H } } { \mathsf { L } } { \mathsf { E N } } { = } 1 , { \mathsf { C } } { \mathsf { K } } { \mathsf { P } } { \mathsf { L } } { = } 1 )$


![image](images/ab7000824d57.jpg)



图 21-46. PCM 标准长帧同步模式时序图 $( { \bf D } { \bf T } { \bf L } { \bf E } { \bf N } { = } { \bf 0 } 1 , { \bf C } { \bf H } { \bf L } { \bf E } { \bf N } { = } 1 , { \bf C } { \bf K } { \bf P } { \bf L } { = } { \bf 0 } )$


![image](images/5b9084f83f98.jpg)



图 21-47. PCM 标准长帧同步模式时序图 $( { \tt D T L E N } { = } { 0 } 1 , { \tt C H L E N } { = } 1 , { \tt C K P L } { = } 1 )$ ）


![image](images/3d08721a3b9e.jpg)



图 21-48. PCM 标准长帧同步模式时序图 $( { \bf D } { \sf T } { \sf L } { \sf E N } { = } { \bf 0 } 0 , { \sf C } { \sf H } { \sf L } { \sf E N } { = } 1 , { \sf C } { \sf K } { \sf P } { \sf L } { = } 0 )$ ）


![image](images/a51627e1a4dc.jpg)



图 21-49. PCM 标准长帧同步模式时序图 $( { \tt D T L E N } { = } { 0 } 0 , { \tt C H L E N } { = } 1 , { \tt C K P L } { = } 1 )$ ）


![image](images/3ec2371db576.jpg)


## 21.9.2. I2S 时钟


图 21-50. I2S 时钟生成结构框图


![image](images/6e0125d9bbd6.jpg)


I2S 时钟生成器如 21-50. I2S 所示。I2S 接口时钟是通过 SPI_I2SPSC 寄存器的 DIV 位，OF 位和 MCKOEN 位以及 SPI_I2SCTL 寄存器的 CHLEN 位来配置的。I2S比特率可以通过 21-7. I2S 所示的公式计算。
...
所以，为了得到期望的音频采样率，时钟生成器需要按 21-8. 所列的公式进行配置。
...
运行模式是通过SPI_I2SCTL寄存器的I2SOPMOD位来选择的。共有四种运行模式可供选择：主机发送模式，主机接收模式，从机发送模式和从机接收模式。各种运行模式下 I2S 接口信号的方向如 21-9.  I2S 所示。


表 21-9. 各种运行模式下 I2S 接口信号的方向


<table><tr><td>运行模式</td><td>I2S_MCK</td><td>I2S_CK</td><td>I2S_WS</td><td>I2S_SD</td><td>I2S_ADD_SD(2)</td></tr><tr><td>主机发送</td><td>输出或<eq>NU^{(1)}</eq></td><td>输出</td><td>输出</td><td>输出</td><td><eq>NU^{(1)}</eq></td></tr><tr><td>主机接收</td><td>输出或<eq>NU^{(1)}</eq></td><td>输出</td><td>输出</td><td>输入</td><td><eq>NU^{(1)}</eq></td></tr><tr><td>从机发送</td><td>输入或<eq>NU^{(1)}</eq></td><td>输入</td><td>输入</td><td>输出</td><td><eq>NU^{(1)}</eq></td></tr><tr><td>从机接收</td><td>输入或<eq>NU^{(1)}</eq></td><td>输入</td><td>输入</td><td>输入</td><td><eq>NU^{(1)}</eq></td></tr><tr><td>全双工</td><td>输出或<eq>NU^{(1)}</eq></td><td>输出</td><td>输出</td><td>输出或输入</td><td>输入或输出</td></tr></table>
...
在从机模式下，从机需要在外部主机开始通讯之前使能。当外部主机开始发送时钟信号且

I2S_WS 信号请求传输数据时，发送流程开始。数据需要在外部主机发起通讯之前写入SPI_DATA 寄存器。为了确保音频数据的连续传输，必须在当前发送序列结束之前将下一个待发送的数据写入 SPI_DATA 寄存器，否则会产生发送欠载错误。此时 TXURERR 标志会置 1，如果 SPI_CTL1 寄存器的 ERRIE 位为 1，将会产生中断。这种情况下，必须先关闭 I2S 再打开 I2S来恢复通讯。从机模式下，I2SCH 标志是根据外部主机发送的 I2S_WS 信号而变化的。
...
在从机模式下，从机需要在外部主机开始通讯之前使能。当外部主机开始发送时钟信号且I2S_WS 信号指示数据开始时，接收流程开始。从机模式下，I2SCH 标志是根据外部主机发送的 I2S_WS 信号而变化的。

为了关闭 I2S，必须在收到最后一个 RBNE 之后立即清除 I2SEN 位。

## I2S全双工模式

单个的 I2S 模块只支持单向传输：发送模式或接收模式，通过一个附加的 I2S 模块（I2S_ADD模块）可以实现 I2S 的全双工模式。I2S_ADD 模块与 I2S模块功能一样，但只工作在从模式。一共有两个 I2S_ADD 模块：I2S_ADD1 和 I2S_ADD2，所以只有 I2S1 和 I2S2 支持全双工模式。I2S_ADD 模块的 I2S_CK 和 I2S_WS 引脚分别与对应的 I2S 模块的相应引脚内部连接，I2S_ADD 模块的 I2S_SD 引脚映射到对应的 I2S 模块的 SPI_MISO 引脚。

为了工作在全双工模式，需要使能 I2S 模块和相应的 I2S_ADD 模块，I2S 模块支持两种全双工模式：主机模式和从机模式。

在主机全双工模式下，软件必须设置 I2S 为主机，I2S_ADD 为从机，I2S_ADD 模块的 WS和SCK 信号都由主机 I2S模块提供。
...
SPI_STAT 寄存器中有 4 个可用的标志位，分别是 TBE、RBNE、TRANS 和 I2SCH，用户通

过这些标志位可以全面监视 I2S总线的状态。
...
21-10. I2S 总结了 I2S中断事件和相应的使能位。


表 21-10. I2S 中断


<table><tr><td>中断事件</td><td>描述</td><td>清除方式</td><td>中断使能位</td></tr><tr><td>TBE</td><td>发送缓冲区空</td><td>写 SPI_DATA 寄存器</td><td>TBEIE</td></tr><tr><td>RBNE</td><td>接收缓冲区非空</td><td>读 SPI_DATA 寄存器</td><td>RBNEIE</td></tr><tr><td>TXURERR</td><td>发送欠载错误</td><td>读 SPI_STAT 寄存器</td><td rowspan="2">ERRIE</td></tr><tr><td>RXORERR</td><td>接收过载错误</td><td>读 SPI_DATA 寄存器,然后再读 SPI_STAT 寄存器</td></tr><tr><td>FERR</td><td>I2S 帧错误</td><td>读 SPI_STAT 寄存器</td><td></td></tr></table>
