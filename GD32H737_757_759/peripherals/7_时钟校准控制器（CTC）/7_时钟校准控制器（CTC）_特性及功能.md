...
CTC模块的内部结构图如 7-1. CTC 。


图 7-1. CTC 简介


![image](images/dbcce4dade85.jpg)
...
$$
\text { RLVALUE } = \left(F _ {\text { clock }} \div F _ {\text { REF }}\right) - 1 \tag {7-1}
$$

CKLIM的值由用户根据时钟的精度来设置，一般建议设为步长的一半，所以CKLIM的值为：

$$
\mathrm{CKLIM} = \left(\mathrm{F} _ {\text { clock }} \div \mathrm{F} _ {\text { REF }}\right) \times 0.12 \% \div 2 \tag{7 - 2}
$$

典型的步长值是0.12%， $F _ { c l o c k }$ 是期望时钟的频率（IRC48M）， $F _ { R E F }$ 是REF同步脉冲信号的频率。
