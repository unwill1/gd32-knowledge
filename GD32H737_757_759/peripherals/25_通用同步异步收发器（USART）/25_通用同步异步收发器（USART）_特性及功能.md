...
传输检测标志：接收缓冲区非空（RBNE）、接收FIFO满（RFF）、接收FIFO空（RFE）、接收FIFO已达阈值（RFT）、发送缓冲区空（TBE）、传输已完成（TC）、发送FIFO未满（TFNF）、发送FIFO空（TFE）、发送FIFO已达阈值（TFTIF）；
...
USART 接口通过 25-1. USART 中主要引脚从外部连接到其他设备。
...
图25-2. USART字符帧（8数据位和1停止位）


![image](images/156ba14affcc.jpg)
...
$$
\text { USARTDIV } = \frac {\text { UCLK }}{1 6 \times \text { Baud   Rate }} \tag {25-1}
$$

如果过采样是8，公式为：

$$
\text { USARTDIV } = \frac {\text { UCLK }}{8 \times \text { Baud   Rate }} \tag {25-2}
$$
...
# 25-3. USART 给出了 USART 发送步骤。软件操作按以下流程进行：
...
图 25-4. 过采样方式接收一个数据位（OSB = 0）


![image](images/7f28dd4ed33f.jpg)
...
当 DMA 用于 USART 发送时，DMA 将数据从片内 SRAM 传送到 USART 的数据缓冲区。配置步骤如 25-5. DMA USART 所示。


图 25-5. 采用 DMA 方式实现 USART 数据发送配置步骤


![image](images/19865843c9cc.jpg)


所有数据帧都传输完成后，USART_STAT寄存器中TC位置1。如果USART_CTL0寄存器中TCIE置位，将产生中断。

当 DMA用于USART 接收时，DMA 将数据从接收缓冲区传送到片内 SRAM。配置步骤如25-6. DMA USART 所示。如果将 USART_CTL2 寄存器中 ERRIE 位置 1，USART_STAT 寄存器中的错误标志位（FERR、ORERR 和 NERR）置位时将产生中断。


图 25-6. 采用 DMA 方式实现 USART 数据接收配置步骤


![image](images/f2983c2dabb1.jpg)
...
图 25-7. 两个 USART 之间的硬件流控制


![image](images/91c4ae33caf9.jpg)
...
断开检测功能完全独立于普通USART接收器。因此，断开检测可以是在空闲状态下，也可以在数据传输过程中。USART_CTL1寄存器中LBLEN位可以选择断开帧的长度。如果在RX引脚检测到大于或等于与预期的断开帧长度的0（LBLEN = 0时，10个0；LBLEN = 1时，11个0），USART_STAT寄存器中LBDF置位。如果USART_CTL1寄存器中LBDIE被置位，将产生中断。如 25-9. 所示，如果断开帧发生在空闲状态下，USART接收器会接收到一个全0数据帧，同时FERR置位。


图 25-9. 空闲状态下检测断开帧


![image](images/098fc8f1ddc5.jpg)



如 25-10. 所示，如果断开帧发生在数据传输过程中，当前传输帧发生错误，FERR 置位。



图 25-10. 数据传输过程中检测断开帧


![image](images/58270c8af74f.jpg)
...
图 25-11. 同步模式下的 USART 示例


![image](images/70daa8a230a7.jpg)



图 25-12. 8-bit 格式的 USART 同步通信波形（CLEN = 1）


![image](images/6eeaa34579f1.jpg)


# 25.3.10. 串行红外（IrDA SIR）编解码功能模块
...
图 25-13. IrDA SIR ENDEC 模块


![image](images/0a631d4fa18a.jpg)
...
![image](images/5e5a227dcee2.jpg)


将USART_CTL2寄存器中IRLP置位可以使SIR子模块工作在低功耗模式下。发送编码器由PCLK分频得到的低速时钟来驱动。分频系数在USART_GP寄存器中PSC[7:0]位配置，USART_BAUD寄存器需配置为16*PSC[7:0]。TX引脚脉冲宽度可以为低功耗波特率的3倍。接
...
半双工模式下仅用单线通信，TX引脚和RX引脚从内部连接到一起，RX引脚不再使用。TX引脚应被配置为开漏模式，通信冲突由软件处理。当TEN被置位时，在数据寄存器中的数据将会被发送。

# 25.3.12. 智能卡（ISO7816-3）模式
...
图 25-15. ISO7816-3 数据帧格式


![image](images/963e8bbffb9e.jpg)
...
图 25-16. USART 接收 FIFO 结构


![image](images/b4c8eb9c2709.jpg)
...
通过将USART_FCS寄存器的FEN置位使能发送FIFO，发送FIFO和发送缓存区可储存多至16帧的数据。若发送FIFO空，TFE将被置位，如果TFEIE位被置位，将产生中断。若发送FIFO可用空间达到TFTCFG[2:0]位域配置的阈值，TFTIF将被置位（TFT将被清除），如果TFTIE被置位，将产生中断。若发送FIFO非满，TFNF位被置位，如果TFNFIE被置位，将产生中断。


图 25-17. USART 发送 FIFO 结构


![image](images/45bb40fc1107.jpg)


# 25.3.16. 从 Deepsleep 模式唤醒
